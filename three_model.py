...
for i in range(0, len(canoe_vars)):
    ...
    depth_index = (np.abs(depths-depth)).argmin()  # Find the nearest depth to the one requested
    plot_depth = depths[depth_index]

    data1 = data1 * CANOE_SCALE_FACTORS[canoe_vars[i]]
    data2 = data2 * PISCES_CMOC_SCALE_FACTORS[pisces_cmoc_vars[i]]
